...
std::set<std::size_t> determineLineBreaks(const TaggedString& logicalInput,
    ...
    const bool hasServerSuggestedBreaks = logicalInput.rawText().find_first_of(u'\u200b') !=  std::string::npos;
        ...
        if (i < logicalInput.length() - 1) {
            ...
            if (allowsIdeographicBreak || util::i18n::allowsWordBreaking(codePoint)) {
                ...
                potentialBreaks.push_back(evaluateBreak(i+1, currentX, targetWidth, potentialBreaks,
                                                        calculatePenalty(codePoint, logicalInput.getCharCodeAt(i+1), penalizableIdeographicBreak),
                                                        false));
            }
        }
    }
